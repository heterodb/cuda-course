...
kern_bitonic_sorting(int nrooms, float *g_buffer)
{
	...
	for (int scale=0; scale <= max_scale; scale++)
	{
		for (int step=scale; step >= 0; step--)
		{
			...
			{
				// tid は論理的なスレッドID（blockDim.xがnroomより小さい場合
				// でも動作する）
				//
				// 論理的なスレッドIDである tid を元に、
				// - 大小比較の方向（direction）
				// - 比較のベースとなる配列インデックス（index）
				// - もう一方の比較要素のインデックス（buddy）
				// を導出する。
				//
				bool	direction = ...;
				int		index = ...;
			...
			// この scale / step における g_buffer の更新が完了するまで
			// スレッドを同期する。
			__syncthreads();
		}
	}	
